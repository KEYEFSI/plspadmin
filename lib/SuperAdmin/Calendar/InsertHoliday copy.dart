...
class _AddHolidayState extends State<AddHolidayState> {
  ...
  final HolidayInsert _holidayInsert = HolidayInsert('$kUrl');
   ...
   HolidayDateController holidayDateController = HolidayDateController(); // Assuming you have this controller
  ...
  bool selectableDayPredicate(DateTime date) {
    ...
    // Disable holidays
    if (holidayDateController.holidayDates.any(
        (holiday) => isSameDate(
            date,
            DateTime.parse(holiday.date.toIso8601String())))) {
      return false;
    }
              ...
              children: [
                ...
                Row(
                  children: [
                    Padding(
                      padding:
                          const EdgeInsetsDirectional.fromSTEB(12, 0, 24, 0),
                      child: Text(
                        _selectedDate != null
...
while (!selectableDayPredicate(initialDate)) {
  // Increment the date until a valid date is found
  initialDate = initialDate.add(Duration(days: 1));
}

// Now you can use initialDate for the date picker
                        ...
                        if (_selectedDate != null &&
                          ...
                          try {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content:
                                    Text('Holiday date saved successfully.'),
                              ),
                            );
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('Failed to save holiday date.'),
                              ),
                            );
                          }
                        } else {
