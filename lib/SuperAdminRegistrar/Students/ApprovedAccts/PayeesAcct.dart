...
class PayeesAccounts extends StatefulWidget {
  const PayeesAccounts({
    super.key,
  });

  @override
  State<PayeesAccounts> createState() => _PayeesAccountsState();
}

class _PayeesAccountsState extends State<PayeesAccounts> {
  
  late PayeesStudentController _controller = PayeesStudentController();
  final _Delcontroller = DeleteStudentController();
  String searchQuery = '';
  int currentPage = 1;
  int itemsPerPage = 10;
  int hoveredIndex = -1;
  ...
  List<PayeesStudent> _paginate(List<PayeesStudent> students) {
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Expanded(
              child: StreamBuilder<List<PayeesStudent>>(
  ...
  void _deleteStudent(
    ...
    final request = DeleteStudentRequest(
      username: username,
      reason: reason,
    );

    final response = await _Delcontroller.deleteStudent(request);
  ...
  void _showReasonDialog(String username) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Would you provide various reason to delete the account.',
                  style: TextStyle(
      ...
      if (reason != null && reason.isNotEmpty) {
        _deleteStudent(context, username, reason);
      } else {
        _showErrorMessage('Reason is required to reject the document.');
      }
    });
  }
