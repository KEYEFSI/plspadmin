...
class _CalendarState extends State<Calendar> {
  ...
  Map<DateTime, RequestCountsByDate> _requestCountsByDateMap = {};
  ...
  Map<DateTime, List<String>> _formatEvents(List<HolidayEvent> holidayDates) {
    final Map<DateTime, List<String>> data = {};
    for (var event in holidayDates) {
      final date = DateTime(event.date.toLocal().year, event.date.toLocal().month, event.date.toLocal().day);
      if (data[date] == null) {
        data[date] = [];
      }
  ...
  Map<DateTime, RequestCountsByDate> _formatRequestCounts(CombinedData combinedData) {
    final Map<DateTime, RequestCountsByDate> aggregatedData = {};

    void addToAggregatedData(List<RequestCountsByDate> requestList) {
      for (var request in requestList) {
        final dateKey = DateTime(request.date.toLocal().year, request.date.toLocal().month, request.date.toLocal().day);
        if (aggregatedData.containsKey(dateKey)) {
          final existing = aggregatedData[dateKey]!;
          aggregatedData[dateKey] = RequestCountsByDate(
            date: dateKey,
    ...
    addToAggregatedData(combinedData.graduatesRequestsByDate);
    addToAggregatedData(combinedData.collegeRequestsByDate);
    addToAggregatedData(combinedData.isRequestsByDate);

    return aggregatedData;
  }

  // Method to get request counts for a specific day
  RequestCountsByDate? _getRequestCountsForDay(DateTime day) {
    final localDay = DateTime(day.year, day.month, day.day);
                  ...
                  calendarBuilders: CalendarBuilders(
                    defaultBuilder: (context, day, focusedDay) {
                      ...
                      final backgroundColor = events.isNotEmpty
                          ? (isPast ? Colors.redAccent : Colors.redAccent.shade700)
                          : Colors.white;
                    ...
                    todayBuilder: (context, day, focusedDay) {
                      ...
                      return _buildDayCell(
                        ...
                        FontWeight.w600,
                        requestCounts,
                        backgroundColor: hasEvents ? null : Colors.green.shade900,
                    ...
                    disabledBuilder: (context, day, focusedDay) {
                      ...
                      final backgroundColor =
                          events.isNotEmpty ? Colors.red.shade100 : Colors.white;
...
Widget _buildDayCell(
  ...
  RequestCountsByDate? requestCounts,  // This is now a nullable parameter
  {
...
final unpaidPercentage = (correspondingData?.unpaidRequests?? 0 / maxRequests) ;
final paidPercentage = (correspondingData?.paidRequests ?? 0/ maxRequests) ;
final percentage = paidPercentage + unpaidPercentage;
