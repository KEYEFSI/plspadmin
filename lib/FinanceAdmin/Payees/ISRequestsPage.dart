import 'package:plsp/FinanceAdmin/Payees/Counter.dart';
...
import 'package:plsp/FinanceAdmin/Payees/ViewProfile.dart';
...
class PayeesRequestPage extends StatefulWidget {
  final String username;
  final String fullname;

  const PayeesRequestPage(
      {super.key, required this.username, required this.fullname});

  @override
  State<PayeesRequestPage> createState() => _PayeesRequestPageState();
}

class _PayeesRequestPageState extends State<PayeesRequestPage> {
  late FMSRISRequestCountController _controller;
  late Future<ISRequestsCount> _requestsCount;
  late FMSRISStudentCountController _controllers;
  late Future<ISStudentCount> _studentCount;
  late FMSRISTransactionCountController _controllert;
  late Future<ISStudentTransactionCount> _transactionCount;

  late TextEditingController _SearchController;
  late FocusNode _SearchFocusNode;
  List<dynamic> _allStudents = [];
  late Future<List<dynamic>> _students = Future.value([]);
  late final GetIS _getIS;
  ...
  void _fetchStudents() async {
    try {
      final fetchedStudents = await _getIS.fetchStudentRequests();
  ...
  void _showStudentDetails(ISStudent student) {
    setState(() {
                          ...
                          Expanded(
                            child: Padding(
                              padding: EdgeInsets.only(
                                  top: height / 42, left: fontsize / 80),
                              child: Text(
                                'All Integrated School Requests',
                                style: GoogleFonts.poppins(
                                  fontSize: fontsize / 80,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.green.shade900,
