...
class RequestCountsTodayController {
  final String apiUrl = '$kUrl/FMSR_GetCollegeStudentsAndPaidDocumentsCount';  // Updated API URL
  Timer? _timer;
  ...
  // Dispose to cancel the timer and close the stream when the controller is no longer needed
  void dispose() {
    _timer?.cancel();
    _controller.close();
  }
...
class RequestStatsController {
  final String apiUrl = '$kUrl/FMSR_GetClaimableAndClaimedCountsForToday';  
  Timer? _timer;
...
class HolidayDatesController {
  final String apiUrl = '$kUrl/FMSR_GetRequestAndClaimedData'; // Updated endpoint URL
  Timer? _timer;
  ...
  Future<void> _fetchAndUpdate() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Parse combined data (requestsByDate, holidayDates, and todayData)
